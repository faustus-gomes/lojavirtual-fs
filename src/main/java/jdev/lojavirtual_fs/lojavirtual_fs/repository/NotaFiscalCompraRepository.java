...
public interface NotaFiscalCompraRepository extends JpaRepository<NotaFiscalCompra, Long> {
    ...
    @Query("select a from NotaFiscalCompra a " +
            "where upper(trim(a.descricaoObs)) like %?1%")
    ...
    @Query(nativeQuery = true, value = "delete from nota_item_produto " +
    ...
    @Query(nativeQuery = true, value = "select count(1) > 0 " +
                                        "from nota_fiscal_compra " +
                                        "where descricao_obs like %?1%")
